...
class SegmentationLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        runway_true = y_true[:, 0, :, :]
        aiming_point_true = y_true[:, 1, :, :]
        threshold_true = y_true[:, 2, :, :]
...
class CombinedLoss(nn.Module):
    def __init__(self, penalty_rate=2.0):
        ...
        self.line_loss = FeatureLineLoss()
    
    def forward(self, y_pred, y_true):
        seg_pred, line_pred = y_pred
        seg_true, line_true = y_true
        seg_loss_val = self.seg_loss(seg_pred, seg_true)
        line_loss_val = self.line_loss(line_pred, line_true)
        return seg_loss_val + line_loss_val